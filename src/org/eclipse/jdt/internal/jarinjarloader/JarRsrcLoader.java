...
/*     */ {
	...
	/*     */ {
		...
		/* 47 */ for (int i = 0; i < mi.rsrcClassPath.length; i++) {
			...
			/* 49 */ if (rsrcPath.endsWith("/")) {
				...
				/*     */ } else/* 52 */ rsrcUrls[i] = new URL("jar:rsrc:" + rsrcPath + "!/");
			/*     */ }
	...
	/*     */ {
		/* 63 */ Enumeration resEnum = Thread.currentThread().getContextClassLoader()
...
/*
 * Location:
 * C:\all\velocity\sql-tool-buddy\sql-tool-buddy.jar!\org\eclipse\jdt\internal\
 * jarinjarloader\JarRsrcLoader.class Java compiler version: 1 (45.3) JD-Core
 * Version: 0.7.1
 */